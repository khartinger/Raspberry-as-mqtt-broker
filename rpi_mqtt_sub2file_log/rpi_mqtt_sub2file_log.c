...
// where yy is the year and mm is the month e.g. mqtt1803.log
// * optional args: -q = quiet (no output to screen), ip broker
// * A topic rpi_mqtt_sub2file with payload ? effects a message
//   with same topic and payload version number of this program
 ...
 const struct mosquitto_message *message)
{
 ...
 sprintf(filename_,"%smqtt%s.log",_PATH_LOG_,sNow);
...
void print_help()
{
 fprintf(stdout, "\nUsage  : rpi_mqtt_sub2file_log [-q] [ip]\n");
 ...
 fprintf(stdout, "All messages are added to a mqttYYMM.log file.\n");
